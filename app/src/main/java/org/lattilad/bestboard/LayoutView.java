...
public class LayoutView extends View
    ...
    public void setLayout(Layout layout)
        ...
        if ( this.layout != layout)
            ...
            // TODO main touch - what to do with main touch - this is not completely clear
            // mainTouchBow = new MainTouchBow( Layout.EMPTY_TOUCH_CODE ); //mainTouchBow() becomes invalid

            // packetJump (where main touch changes layout) should break main touch -
            // because it cannot continue on the same button on the next layout
             ...
             *
             * But what does happen if button index (below this touch) changes on the new layout?
             * It could generate a new touch - even if button is the same on the new index.
             *
             * MultiTouchBow.touchCode cannot be EMPTY - because if touch reaches an EMPTY space,
             * multi button is released (and touch is removed)
             *
             * Writing EMPTY into multiTouchBow.touchCode means: this is an invalid touch on a new
             * layout - the new touchCode (on the same position on new layout) should replace
             * previous touchCode
             *
             * This can happen only if layout is changed, all other multitouch buttons cannot change
             * their underlaying touch code.
             */
            ...
            // Sister buttons should be refreshed on the selected layout - check SisterButton interface!
            this.layout.refreshSisterButton( null );
            ...
            // clears strokePaths ?? not strokePoints?
            //strokePaths.clear();
            strokePoints.clear();
            }
    ...
    private class MainTouchBow
        ...
        MainTouchBow()
            {
            touchCode = Layout.EMPTY_TOUCH_CODE;
            buttonMainTouch = null;
            }

        MainTouchBow( int touchCode )
            {
            this( touchCode, null, null);
            }

        MainTouchBow( int touchCode, ButtonMainTouch buttonMainTouch, StrokePoint strokePoint )
            {
            ...
            if ( buttonMainTouch != null )
                ...
                // undo is restricted to one button (one bow) - previous undo is cleared here
                layout.softBoardData.softBoardListener.clearUndoString();
                ...
                if ( isSticky() )
                    {
                    // origin is needed only by STICKY buttons
                    this.originStrokePoint = strokePoint; // strokePoint is NEVER null in this case
                    Scribe.debug(Debug.TOUCH, "Origin X: " + strokePoint.canvasX + " Y: " + strokePoint.canvasY);
                    }
                else
                    {
        ...
        // parameters used by STICKY buttons
        StrokePoint originStrokePoint;

        /** true if this bow contains stickybutton */
        boolean isSticky()
            {
            return buttonMainTouch instanceof ButtonSticky;
            }

        StrokePoint getOrigin()
            {
            return originStrokePoint;
            }
        }
    ...
    private void _touchEventsHoldAndMove(int canvasX, int canvasY, float canvasPressure)
        ...
        if ( strokePoints.size() > 0 && // after setLayout() stroke can start with "move" - it will be HOLD
            ...
            {
            // MOVE on STICKY is different - cannot leave button by move

            // repeatHandler is started by mainTouchBow, so it is working
            // BUT
            // no further strokePath,
            // no long move (on circle)
            // no pressure check

            if ( !evaluateSticky(strokePoint) )
                {
    ...
    public void breakMainTouch()
        {
        mainTouchBow = new MainTouchBow( Layout.EMPTY_TOUCH_CODE ); //mainTouchBow() becomes invalid
        strokePointerId = -1; // from now on new DOWN touch is needed, to start a new bow
        strokePoints.clear(); // strokeline is cleared
        // what about strokePaths? should it be also cleared?
        // what about button itself? should it be finished?
        }
    ...
    private void evaluateMain(int bowAction, StrokePoint strokePoint)
        ...
        if (mainTouchBow.touchCode != newBowTouchCode)
            ...
            if (mainTouchBow.buttonMainTouch != null)
                ...
                typeMetaState();
                // TODO some buttons (BACKSPACE, TEXT packet release) clear META states expicitly - is it really needed?
                // TODO should it be BEFORE or AFTER mainTouchEnd? (ButtonSticky sends packet at mainTouchEnd)
                ...
                // If board was changed NO "type" emulation is needed
            ...
            if (newBowTouchCode != Layout.EMPTY_TOUCH_CODE)
                ...
                if (newBowButton instanceof ButtonMainTouch)
                    ...
                    // STICKY this is the only starting point for ButtonMainTouch
                    // not only StickyButton-s, but all bows will get bow origin
                    mainTouchBow = new MainTouchBow( newBowTouchCode, (ButtonMainTouch)newBowButton, strokePoint );

                    // TODO - CHECK IT FIRST !!! (DOUBLE MUTE - "string" clears previous string) !!!
                    // STICKY - is it possible to clear undo string at the beginning of each bow???
                    // layout.softBoardData.softBoardListener.clearUndoString();
                    ...
                    // TIMING EVENT; could be in mainTouchStartCall
            ...
            mainTouchBow = new MainTouchBow( newBowTouchCode );
            }
        else // same bow
            {
            // check bow's long
    ...
    // !! This method (or even evaluateMain) could go into "bow"-s class ??

    /**
     * Each bow stores its origin for sticky buttons.
     * If button is STICKY, then MOVE (inside _touchEventsAndHold) will come here
     * (and not into evaluateMain) - so MOVE cannot finish STICKY buttons
     * UP (and CANCEL) will finish them through evaluateMain()
     */
    private boolean evaluateSticky( StrokePoint strokePoint )
        {
        Scribe.debug(Debug.TOUCH, "STICKY X: " + strokePoint.canvasX +
                " Y: " + strokePoint.canvasY);

        if ( mainTouchBow.isSticky() )
            {
            int repeat = ((ButtonSticky)(mainTouchBow.buttonMainTouch)).mainTouchSticky(
                    strokePoint.canvasX - mainTouchBow.originStrokePoint.canvasX,
                    strokePoint.canvasY - mainTouchBow.originStrokePoint.canvasY );
            if ( repeat > 0 ) // START REPEAT !!
                {
                //Scribe.debug("REPEAT STARTED");
                repeatHandler.postDelayed(repeatRunnable, layout.softBoardData.repeatTime);
                // it will call MainTouchSecondaryCall( ON_STAY ) and NOT MainTouchSticky
                // MainTouchSecondaryCall( ON_STAY ) should return true, to repeat at repeatTime
                // MOVE will call mainTouchSticky - it can stop repeat returning false
                }
            else if ( repeat < 0 ) // STOP REPEAT !!
                {
                //Scribe.debug("REPEAT STOPPED");
                repeatHandler.removeCallbacks( repeatRunnable );
                }
            // else repeat == 0 // DO NOTHING !!
