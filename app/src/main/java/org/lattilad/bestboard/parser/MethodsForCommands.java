...
import org.lattilad.bestboard.buttons.ButtonSticky;
import org.lattilad.bestboard.buttons.ButtonStickyJoy;
...
import org.lattilad.bestboard.buttons.PacketJump;
import org.lattilad.bestboard.buttons.PacketKey;
import org.lattilad.bestboard.buttons.PacketLoad;
import org.lattilad.bestboard.buttons.PacketModify;
...
public class MethodsForCommands
    ...
    private class ButtonExtension
        ...
        // Clear previous titles, defined by button (or let, or define)
        // It is useful, if previously defined arrangement should be rearranged
        private boolean clearTitles = false;

        // Can be TOKEN_DOUBLE TOKEN_ALTERNATE TOKEN_MULTI TOKEN_LIST
        // TOKEN_STICKY TOKEN_JOY
        private Long type = null;
        // SECONDARY packet (DOUBLE OR ALTERNATE)
        // List of ADD packets (MULTI)
        // List of ADDTEXT packets (LIST)
        // Packet[][] STICKY (repeat should be set, too!)
        // Packet[][] JOY
                ...
                if ( button == null )
                    ...
                    if ( buttonExtension.type != null )
                        ...
                        if (button instanceof ButtonSingle)
                            ...
                            else if (buttonExtension.type == Commands.TOKEN_STICKY)
                                {
                                button = ((ButtonSingle) button)
                                        .extendToSticky((Packet[][]) buttonExtension.data, buttonExtension.repeat);
                                }
                            else if (buttonExtension.type == Commands.TOKEN_JOY)
                                {
                                button = ((ButtonSingle) button)
                                        .extendToJoy((Packet[][]) buttonExtension.data);
                                }
                    ...
                    if ( buttonExtension.titleList != null )
                        {
                        // previous titles can be inherited or omitted - if cleartitles is set to null
                        // it only happens if new titles are defined
                        // TODO - cleartitles could work without new titles - but default title should be defined
                        SinglyLinkedList<TitleDescriptor> titles =
                                buttonExtension.clearTitles ?
                                new SinglyLinkedList<TitleDescriptor>() :
                                new SinglyLinkedList<TitleDescriptor>(button.getTitles());
    ...
    public Packet packetFunction( ExtendedMap<Long, Object> parameters )
        ...
        if ((temp = parameters.remove(Commands.TOKEN_JUMP)) != null)
            return (PacketJump)temp;

        if ((temp = parameters.remove(Commands.TOKEN_MOD)) != null)
            return (PacketModify)temp;
    ...
    // Creates PacketModify from complex parameter
    public PacketModify packetModify(ExtendedMap<Long, Object> parameters )
        {
        Long rollId = (Long) parameters.remove( Commands.TOKEN_ROLL );
        if ( rollId == null )   return null;

        return new PacketModify( softBoardData,
                rollId, parameters.remove( Commands.TOKEN_REVERSE ) != null);
        }


    public PacketJump packetJump(ExtendedMap<Long, Object> parameters )
        {
        ...
        return new PacketJump( softBoardData, boardId,
    ...
    public Button setMemory( ExtendedMap<Long, Object> parameters )
        ...
        Scribe.debug(Debug.DATA, "Memory Button (" +
                ( id == null ? "without id" : Tokenizer.regenerateKeyword(id)) + ") is defined");

        // Packet with default cannot be null!
        return completeMainTouchButton( new ButtonMemory( id,
    ...
    // if FIRST parameter is given, then FIRST token is searched
    // if SECOND parameter is given, then SECOND token is searched
    private Packet[] stickyCenterPacket( ExtendedMap<Long, Object> firstParameters,
                                         ExtendedMap<Long, Object> secondParameters )
        {
        Packet[] centerPacketArray = new Packet[ButtonSticky.CENTER_SIZE];
        if ( firstParameters != null )
            centerPacketArray[ButtonSticky.FIRST] = firstPacket( firstParameters );
        if ( secondParameters != null  )
            centerPacketArray[ButtonSticky.SECOND]
                    = (Packet) secondParameters.remove(Commands.TOKEN_SECOND);

        return  centerPacketArray;
        }

    private Packet[] stickyCompassPacket( ExtendedMap<Long, Object> parameters )
        {
        Packet[] compassPacketArray = new Packet[ButtonSticky.COMPASS_SIZE];
        compassPacketArray[ButtonSticky.NORTH] = (Packet)parameters.remove( Commands.TOKEN_NORTH );
        compassPacketArray[ButtonSticky.EAST] = (Packet)parameters.remove( Commands.TOKEN_EAST );
        compassPacketArray[ButtonSticky.SOUTH] = (Packet)parameters.remove( Commands.TOKEN_SOUTH );
        compassPacketArray[ButtonSticky.WEST] = (Packet)parameters.remove( Commands.TOKEN_WEST );
        return  compassPacketArray;
        }

    private Packet[] stickyHexagonalPacket( ExtendedMap<Long, Object> parameters )
        {
        Packet[] hexagonalPacketArray = new Packet[ButtonSticky.HEXAGONAL_SIZE];
        hexagonalPacketArray[ButtonSticky.R] = (Packet)parameters.remove( Commands.TOKEN_HEXR );
        hexagonalPacketArray[ButtonSticky.DR] = (Packet)parameters.remove( Commands.TOKEN_HEXDR );
        hexagonalPacketArray[ButtonSticky.DL] = (Packet)parameters.remove( Commands.TOKEN_HEXDL );
        hexagonalPacketArray[ButtonSticky.L] = (Packet)parameters.remove( Commands.TOKEN_HEXL );
        hexagonalPacketArray[ButtonSticky.UL] = (Packet)parameters.remove( Commands.TOKEN_HEXUL );
        hexagonalPacketArray[ButtonSticky.UR] = (Packet)parameters.remove( Commands.TOKEN_HEXUR );
        return  hexagonalPacketArray;
        }

    private Packet[] stickyHexedgePacket( ExtendedMap<Long, Object> parameters )
        {
        Packet[] hexagonalPacketArray = new Packet[ButtonSticky.HEXEDGE_SIZE];
        hexagonalPacketArray[ButtonSticky.U] = (Packet)parameters.remove( Commands.TOKEN_HEXR );
        hexagonalPacketArray[ButtonSticky.RU] = (Packet)parameters.remove( Commands.TOKEN_HEXDR );
        hexagonalPacketArray[ButtonSticky.RD] = (Packet)parameters.remove( Commands.TOKEN_HEXDL );
        hexagonalPacketArray[ButtonSticky.D] = (Packet)parameters.remove( Commands.TOKEN_HEXL );
        hexagonalPacketArray[ButtonSticky.LD] = (Packet)parameters.remove( Commands.TOKEN_HEXUL );
        hexagonalPacketArray[ButtonSticky.LU] = (Packet)parameters.remove( Commands.TOKEN_HEXUR );
        return  hexagonalPacketArray;
        }

    private Packet[] stickyClocklikePacket( ExtendedMap<Long, Object> parameters )
        {
        Packet[] clocklikePacketArray = new Packet[ButtonSticky.CLOCKLIKE_SIZE];
        clocklikePacketArray[0]  = (Packet)parameters.remove( Commands.TOKEN_CLOCK1 );
        clocklikePacketArray[1]  = (Packet)parameters.remove( Commands.TOKEN_CLOCK2 );
        clocklikePacketArray[2]  = (Packet)parameters.remove( Commands.TOKEN_CLOCK3 );
        clocklikePacketArray[3]  = (Packet)parameters.remove( Commands.TOKEN_CLOCK4 );
        clocklikePacketArray[4]  = (Packet)parameters.remove( Commands.TOKEN_CLOCK5 );
        clocklikePacketArray[5]  = (Packet)parameters.remove( Commands.TOKEN_CLOCK6 );
        clocklikePacketArray[6]  = (Packet)parameters.remove( Commands.TOKEN_CLOCK7 );
        clocklikePacketArray[7]  = (Packet)parameters.remove( Commands.TOKEN_CLOCK8 );
        clocklikePacketArray[8]  = (Packet)parameters.remove( Commands.TOKEN_CLOCK9 );
        clocklikePacketArray[9]  = (Packet)parameters.remove( Commands.TOKEN_CLOCK10 );
        clocklikePacketArray[10] = (Packet)parameters.remove( Commands.TOKEN_CLOCK11 );
        clocklikePacketArray[11] = (Packet)parameters.remove( Commands.TOKEN_CLOCK12 );
        return  clocklikePacketArray;
        }

    private Packet[][] stickyCollectPackets( Packet[] centralPacket,
                                             Packet[] compassPacket,
                                             Packet[] hexagonalPacket,
                                             Packet[] hexedgePacket,
                                             Packet[] clocklikePacket )
        {
        Packet[][] packetArray = new Packet[ButtonSticky.SIZE][];
        packetArray[ButtonSticky.CENTER_PACKET] = centralPacket;
        packetArray[ButtonSticky.COMPASS_PACKET] = compassPacket;
        packetArray[ButtonSticky.HEXAGONAL_PACKET] = hexagonalPacket;
        packetArray[ButtonSticky.HEXEDGE_PACKET] = hexagonalPacket;
        packetArray[ButtonSticky.CLOCKLIKE_PACKET] = clocklikePacket;
        return packetArray;
        }


    // !!! TODO toSticky should be definied !!!
    public Button setSticky( ExtendedMap<Long, Object> parameters )
        {
        Scribe.debug(Debug.DATA, "Sticky Button is defined");


        /*
        int index;
        for (index = 1 ; index < packetArray.length ; index++ ) // CENTER is NOT checked!!
            {
            if ( packetArray[index] != null ) break;
            }

        if ( index == packetArray.length ) // All directions are null
            {
            if ( packetArray[ButtonSticky.CENTER] == null ) // no packets at all
                return setEmpty( parameters );

            // no pocets for directions - it behave as a normal packet, REPEAT is omitted
            return completeMainTouchButton(
                    new ButtonSingle(packetArray[ButtonSticky.CENTER], ButtonSingle.CAPITAL), // CAPITAL is the default behavior (==0)
                    parameters);
            }
        */

        // Sticky is NOT a regular MainTouchButton, so ON_STAY and ON_CIRCLE cannot be used!!
        // return completeMainTouchButton(new ButtonSticky(...), parameters);
        return completeButton(
                new ButtonSticky( stickyCollectPackets(
                        stickyCenterPacket(parameters, parameters),
                        stickyCompassPacket(parameters),
                        stickyHexagonalPacket(parameters),
                        stickyHexedgePacket(parameters),
                        stickyClocklikePacket(parameters)),
                ( parameters.remove(Commands.TOKEN_REPEAT) != null ) ), parameters);
        }


    // !!! TODO toStickyJoy should be definied !!!
    public Button setStickyJoy( ExtendedMap<Long, Object> parameters )
        {
        Scribe.debug(Debug.DATA, "Sticky Joystick Button is defined");
        ...
        if ( index == packetArray.length ) // All directions are null
            ...
            // no pockets for directions - it behave as a normal packet
            return completeMainTouchButton(
                    new ButtonSingle(packetArray[ButtonSticky.CENTER], ButtonSingle.CAPITAL), // CAPITAL is the default behavior (==0)
                    parameters);
            }
        */
        ...
        // return completeMainTouchButton(new ButtonSticky(...), parameters);
        return completeButton(
                new ButtonStickyJoy( stickyCollectPackets(
                        stickyCenterPacket(parameters, null),
                        stickyCompassPacket(parameters),
    ...
    public ButtonExtension extendButton( ExtendedMap<Long, Object> parameters )
        ...
        buttonExtension.clearTitles = ( parameters.remove( Commands.TOKEN_CLEARTITLES ) != null );

        // ADD can be (MULTI), where MULTI is not obligatory
        if ( (buttonExtension.data = parameters.remove( Bit.setSignedBitOn(Commands.TOKEN_ADD) )) != null )
            {
        ...
        else if ( parameters.remove(Commands.TOKEN_TOSTICKY) != null )
            {
            buttonExtension.type = Commands.TOKEN_STICKY;
            // CENTRAL packet is defined by original BUTTON
            buttonExtension.data = stickyCollectPackets(
                    stickyCenterPacket(null, parameters), // First should not be used!!
                    stickyCompassPacket(parameters),
                    stickyHexagonalPacket(parameters),
                    stickyHexedgePacket(parameters),
                    stickyClocklikePacket(parameters));
            buttonExtension.repeat = ((parameters.remove( Commands.TOKEN_REPEAT )) != null);

            // Scribe.error("TOKEN_REPEAT in TOSTICKY: " + buttonExtension.repeat );
            }
        else if ( parameters.remove(Commands.TOKEN_TOJOY) != null )
            {
            buttonExtension.type = Commands.TOKEN_JOY;
            // CENTRAL packet is defined by original BUTTON
            buttonExtension.data = stickyCollectPackets( stickyCenterPacket(null, null), // Nor FIRST nor SECOND packet is allowed!!
                    stickyCompassPacket(parameters), null, null,null);
            }
        // SECOND can be ALTERNATE or (DOUBLE), where DOUBLE is not obligatory
        // IMPORTANT!! SECOND && TOSTCKY means extend to sticky,
        // but SECOND alone means extend to DOUBLE - so EXTEND TO DOUBLE should be the last in this list!
        else if ( (buttonExtension.data = parameters.remove( Commands.TOKEN_SECOND )) != null )
            {
