...
public class ButtonSingle extends ButtonMainTouch implements Cloneable
    ...
    public ButtonSticky extendToSticky( Packet[][] packetArrays, boolean repeat )
        {
        ButtonSticky buttonSticky;

        packetArrays[ButtonSticky.CENTER_PACKET][ButtonSticky.FIRST] = packet;
        buttonSticky = new ButtonSticky( packetArrays, repeat);
        buttonSticky.setTitles( getTitles() );
        buttonSticky.setColor( color );

        return buttonSticky;
        }


    public ButtonStickyJoy extendToJoy( Packet[][] packetArrays )
        {
        ButtonStickyJoy buttonStickyJoy;

        packetArrays[ButtonSticky.CENTER_PACKET][ButtonSticky.FIRST] = packet;
        buttonStickyJoy = new ButtonStickyJoy( packetArrays );
        buttonStickyJoy.setTitles( getTitles() );
        buttonStickyJoy.setColor( color );

        return buttonStickyJoy;
        }
